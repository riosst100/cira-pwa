...
export default function SearchStore({ query }) {
    const { data } = getStore(query);
    const stores = data ? data.reduce((acc, val) => [...acc, ...val.stores], []) : [];

    return (
        <>
            <hr />
            <div className="p-2"><b>Toko</b><span style={{"float":"right","color":"grey","fontSize":"12px", "paddingTop":"2px"}}>{stores.length} hasil</span></div>
            <hr />
            <div>
                {
                    stores ? (
...
export function getStore(query) {
    return useSWRInfinite(() => {
        return `/api/shop/store/search?q=${query}`;
    }, fetcher, {});
}

export function getStoreById(store_id) {
    return useSWRInfinite(() => {
        return `/api/shop/getStoreById?store_id=${store_id}`;
    }, fetcher, {});
}
